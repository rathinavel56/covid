...
$app->POST('/api/v1/users/login', function ($request, $response, $args) {
	...
	if (USER_USING_TO_LOGIN == 'username') {
		...
		$log_user = $user->where('mobile', $body['mobile'])->with($enabledIncludes)->where('is_active', 1)->where('is_email_confirmed', 1)->first();
	}
...
$app->GET('/api/v1/admin/appointments', function ($request, $response, $args) {
    ...
    try {
	    ...
	    $respones = Models\ScheduleTest::with('user')->Filter($queryParams)->paginate($count);
		$respones = $respones->toArray();
        $data = $respones['data'];
        unset($respones['data']);
        $result = array(
